...
const PARALLEL_URLS_TO_SYNC = 2;
const CONTEXT_TABLE_WRITE_CAPACITY = 1;
const CONTEXT_TABLE_READ_CAPACITY = 1;
...
export const queuePaths = async (contextTableName: string, paths: string[]) => {
    ...
    for (const pathGroup of _.chunk(paths, 20)) {
        ...
        await Promise.all(pathGroup.map(async (newPath: string) => queuePath(contextTableName, newPath)));
    }
};
